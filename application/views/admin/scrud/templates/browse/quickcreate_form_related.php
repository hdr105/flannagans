...
$c_id = $_GET['com_id'];
?>
<script src="http://localhost/man/media/ckeditor/ckeditor.js"></script>
<!--MODULE START-->
	...
	<input type="hidden" name="existing" id="existing">
			...
			foreach ($sections as $sk => $sv) {
				if ($sv['section_view'] == 'outer') {
					...
					$toggle_class = '';
					if ($tcounter ==1) {
						$tab_main_div = '<div class="panel-group accordion" id="frm_accordion_new">';
						$active_class = 'in';
					} else {
					...
					$tab_content_div .= '<div class="panel panel-default">
						<div class="panel-heading">
							<h4 class="panel-title">
								<a class="accordion-toggle accordion-toggle-styled '.$toggle_class.'" data-toggle="collapse" data-parent="#frm_accordion_new" href="#frm_accordion_new_'.$scounter.'"> '.$sv['section_title'].' </a>
							</h4>
						</div>
						<div id="frm_accordion_new_'.$scounter.'" class="panel-collapse '.$active_class.'">
...
<script>CKEDITOR.replace('dataLegal_letters_dataContent');</script>
<style type="text/css">
