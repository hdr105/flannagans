...
$_p['conditions'] = array('component_table="'.$this->table.'"');
$mrs = $_m->find($_p);
...
<div id="summary_view_container">
	<div class="portlet box green">
		<div class="portlet-title">
			<div class="caption">
				<?php 
				if($this->table=='crud_users')
					echo $mrs[1]['component_name']; 
				else
					echo $mrs[0]['component_name']; 
				?> </div>
		</div>
		<div class="portlet-body">

			<div >
				 <table class="table table-striped table-bordered table-hover table-checkable order-column" id="dtbl_<?php echo $this->summaryData["hidden_controll"]; ?>">
					<thead>
						<tr>
					...
					<tbody>
						<?php
								...
								?>
								<?php 
									$this->summaryData["selected_id"] = $result[$this->table]["id"];
									$module_key = isset($result[$this->table][$this->summaryData["module_key"]]) ? $result[$this->table][$this->summaryData["module_key"]] : '';
									$module_value = isset($result[$this->table][$this->summaryData["module_value"]]) ? $result[$this->table][$this->summaryData["module_value"]] : '';
									if($this->table=='contact')
										$module_value .= ' '.$result[$this->table]['Last_Name'];
									echo '<tr onclick=\'selectedRecord('.json_encode($this->summaryData).');\' style="cursor: pointer;"><input type="hidden" name="record_id_'.$result[$this->table]["id"].'" value="'.$module_key.'"><input type="hidden" name="record_viewfield_'.$result[$this->table]["id"].'" value="'.$module_value.'">';
								?>
