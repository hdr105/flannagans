...
foreach ($this->colsCustom as $k => $v) {
	if ($k == 'action') {
		...
		array_unshift($matches[0], '<a class="btn btn-icon-only yellow fa fa-times" id="rmc_{crud_components.id}" ></a>');
		array_unshift($matches[0], '<a class="btn btn-icon-only grey-cascade fa fa-wrench" href="'.base_url().'index.php/admin/scrud/config?com_id={crud_components.id}" ></a>');
		$clsWidth += 130;
                            ...
                            buttons: [
                                {
                                    ...
                                    addClass: 'btn-danger',
                                    click: function(id, data) {
                                        window.location='<?php echo base_url(); ?>index.php/admin/scrud/removeConfig?com_id='+com_id;
                                        $.sModal('close', id);
                                    }
                                },
                                {
