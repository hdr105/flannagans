...
?>
                        ...
                        <div class="btn-group pull-left" style="margin-right:5px;">
                            <select class="form-control active green siteid" style="background:#32c5d2;  color:#FFF;" id="sel2"<?php echo ($CRUD_AUTH['site_id']!=0 and $CRUD_AUTH['group']['id']>=2 and $CRUD_AUTH['group']['id']!=11)? ' disabled="disabled"': ''; ?>>
                                ...
                                foreach ($sites as $key => $row) {
                                    if(($CRUD_AUTH['group']['id']==11 and $row['id']!=67) and ($CRUD_AUTH['group']['id']==11 and $row['id']!=111))
                                        continue;
                                ?>
    ...
    $('.siteid').on('change', function() {
        $url = "<?=base_url() ?>admin/scrud/status?cfun=setSiteID&site_id="+$('.siteid').val()+"&url=<?=urlencode('http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'])?>";
    ...
    $('.action').on('change', function() {
        $url = "<?=base_url() ?>admin/scrud/status?cfun=working_state";
...
<!-- /////////////////// Module Working  ///////////////-->
                ...
                <?php 
                    ...
                    foreach ($mnuGroup as $v){
                        ...
                        if ($show_menu_group) {
                            $array = array('line-chart','male','calendar','users','folder','briefcase','gears','docs','folder','social-dribble','puzzle');
                            $icom = $array[$k];
                            $k++;
                            if($icom=='')
                                $icom='diamond';
                ...
                ?>
                        <li class="nav-item  ">
                            ...
                            foreach ($v['coms'] as $com){
                                ...
                                ?>
                                <li class="nav-item  ">
                                    <a href="<?php echo base_url(); ?>admin/<?php if($com['id']==83){ ?>profile/work_history<?php }elseif($com['id']==84){ ?>audit<?php }elseif($com['id']==85){ ?>checklist/builder<?php }else{ ?>scrud/browse<?php } if($com['id']!=85){ ?>?com_id=<?php echo $com['id']; } ?>" class="nav-link ">
                ...
                ?>
                            ...
                            foreach ($v['coms'] as $com){
                                ...
                                ?>
                                <li class="nav-item  ">
                                    <a href="<?php echo base_url(); ?>admin/<?php if($com['id']==85){ ?>checklist/builder<?php }elseif($com['id']!=83){ ?>scrud/browse<?php }else{ ?>profile/work_history<?php } ?>?com_id=<?php echo $com['id']; ?>" class="nav-link ">
                                        <i class="fa fa-<?=$icom?>"></i>
                                        <span class="title"><?php echo $com['component_name']?></span>
                                    </a>
                                </li>
