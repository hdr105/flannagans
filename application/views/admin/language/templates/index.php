...
foreach ($this->colsCustom as $k => $v) {
	if ($k == 'action') {
		...
		array_unshift($matches[0], '<a class="btn btn-icon-only grey-cascade fa fa-wrench" href="'.base_url(). 'index.php/admin/language/translate?id={crud_languages.id}"></a>');
        $matches[0][1]="<a href=\"javascript:;\" onclick=\"__view('{ppri}'); return false;\" class=\"btn btn-icon-only blue fa fa-search\"></a>";
        $matches[0][2]="<a href=\"javascript:;\" onclick=\"__edit('{ppri}'); return false;\" class=\"btn btn-icon-only green fa fa-edit\"></a>";
        $matches[0][3]="<a href=\"javascript:;\" onclick=\"__delete('{ppri}'); return false;\" class=\"btn btn-icon-only red fa fa-trash\"></a>";
...
// echo "<pre>";//print_r($matches);print_r($this->colsCustom);print_r($fields); echo count($fields); print_r($this->form);print_r($this->results);exit;
foreach ($this->colsWidth as $k => $v) {
	if ($k == 'action') {
		$this->colsWidth[$k] =  $clsWidth;
	}
}
//echo "<pre>";print_r($matches);print_r($this->colsCustom);exit;
?>
        ...
        <div class="pull-right">
            <div class="btn-group">
                ...
                if (isset($q['xtype']))
                    unset($q['xtype']);
                ?>
                <a class="btn" href="?<?php echo http_build_query($q, '', '&'); ?>"><i class="glyphicon glyphicon-remove-circle"></i></a>
    ...
    <div class='<?php echo $this->conf['color']; ?>'>
        ...
        <form method="post" action="?<?php echo http_build_query($q, '', '&'); ?>" id="table" class="form-horizontal">
            ...
            <div style="overflow: auto;">
                <table class="table table-bordered table-hover list table-condensed table-striped" id="sample_1">
                    <thead>
                        <tr>
            ...
            <div class="row">
            <?php require FCPATH . 'application/views/admin/scrud/templates/browse/paginate.php'; ?>
            </div>
        </form>
    </div>
    </div>
