...
require_once $class_path . '/class/Cfunctions.php';
class Crud {
    ...
    private function genIndexView() {
        ...
        if(isset($_GET['f']) && $_GET['f'] != 0){
            ...
            if(!empty($details->cond) && isset($CRUD_AUTH['site_id'])){
                $conditions .= ' ' . $strAnd . $details->cond.' AND site_id = '. $CRUD_AUTH['site_id'] .' ' ;
                $strAnd = 'AND ';
        ...
        foreach ($this->data[$this->conf['table']] as $k => $v){
            foreach ($this->conf['form_elements'] as $value){
                if (isset($value['section_fields'][$this->conf['table'].'.'.$k]) && 
                    $value['section_fields'][$this->conf['table'].'.'.$k]['element'][0] == 'date'){
            ...
            if ($editFlag) {
                ...
                try {
                    ...
                    if(isset($_POST['calendar_data']) && $_POST['calendar_data'] && $update_id){
                        ...
                        if($module_data[0]['component_table'] != 'calendar'){
                            ...
                            foreach($_POST['calendar_data'][$module_data[0]['component_table']] as $key => $cal_entry_save){
                                ...
                                $data['activitytype']= 0;
                                $data['date_start']= date("Y-m-d", strtotime($_POST['data'][$module_data[0]['component_table']][$key]));
                                $data['due_date']= date("Y-m-d", strtotime($_POST['data'][$module_data[0]['component_table']][$key]));
                                $data['time_start']= '';
                                $data['time_end']= '';
                                ...
                                echo $data['eventstatus']= $_POST['calander_for_data'][$module_data[0]['component_table']][$key];
                                $data['priority']= '';
                                $data['location']= '';
                                $data['notime']= '';
                                $data['visibility']= '1';
                                $data['recurringtype']= '';
                                $data['invite_calendars']= '';
                    ...
                    if($moduleId==41 or $moduleId==42 or $moduleId==43 or $moduleId==44 or $moduleId==45){
                        header("Location: browse?com_id=$moduleId&xtype=form&bname=".$_GET['bname']."&key[business.id]=$update_id");
                    }elseif($moduleId==58)
                        header("Location: browse?com_id=$moduleId&xtype=form&bname=".$_GET['bname']."&key[forms.id]=$update_id");
                    elseif($moduleId==62)
                        header("Location: browse?com_id=$moduleId&xtype=form&bname=".$_GET['bname']."&key[compliance.id]=$update_id");
                    elseif($moduleId==63)
                        header("Location: browse?com_id=$moduleId&xtype=form&bname=".$_GET['bname']."&key[codes.id]=$update_id");
                    elseif($moduleId==64)
                        header("Location: browse?com_id=$moduleId&xtype=form&bname=".$_GET['bname']."&key[business_fee.id]=$update_id");
                    elseif($moduleId==70)
                        header("Location: browse?com_id=$moduleId&xtype=form&bname=".$_GET['bname']."&key[services.id]=$update_id");
                    elseif($moduleId==74)
                        header("Location: browse?com_id=$moduleId&xtype=form&bname=".$_GET['bname']."&key[legal_letters_business.id]=$update_id");
                    else
                        header("Location: ?" . http_build_query($q, '', '&'));
                } catch (Exception $e) {
        ...
        foreach ($ids as $thisID) {
            ...
            foreach ($this->data[$this->conf['table']] as $k => $v){
                ...
                if (isset($this->conf['form_elements'][$sectionCounter]['section_fields'][$this->conf['table'].'.'.$k]) && 
                    $this->conf['form_elements'][$sectionCounter]['section_fields'][$this->conf['table'].'.'.$k]['element'][0] == 'date'){
                    if (__DATE_FORMAT__ == 'dd/MM/yyyy'){
                        $v = str_replace('/','-',$v);
                    }
                    $v = str2mysqltime($v,'Y-m-d');
        ...
        if (count($_POST) > 0 && $this->validate() && $auth_token == $CI->session->userdata('auth_token_xtable')) {
            ...
            if ($editFlag) {
                ...
                try {
                    ...
                    if($moduleId==41 or $moduleId==42 or $moduleId==43 or $moduleId==44 or $moduleId==45)
                        header("Location: browse?com_id=$moduleId&xtype=form&bname=".$_GET['bname']."&key[business.id]=$moduleEntityId");
                    elseif($moduleId==58)
                        header("Location: browse?com_id=$moduleId&xtype=form&bname=".$_GET['bname']."&key[forms.id]=$moduleEntityId");
                    elseif($moduleId==62)
                        header("Location: browse?com_id=$moduleId&xtype=form&bname=".$_GET['bname']."&key[compliance.id]=$moduleEntityId");
                    elseif($moduleId==63)
                        header("Location: browse?com_id=$moduleId&xtype=form&bname=".$_GET['bname']."&key[codes.id]=$moduleEntityId");
                    elseif($moduleId==64)
                        header("Location: browse?com_id=$moduleId&xtype=form&bname=".$_GET['bname']."&key[business_fee.id]=$moduleEntityId");
                    elseif($moduleId==70)
                        header("Location: browse?com_id=$moduleId&xtype=form&bname=".$_GET['bname']."&key[services.id]=$moduleEntityId");
                    elseif($moduleId==74)
                        header("Location: browse?com_id=$moduleId&xtype=form&bname=".$_GET['bname']."&key[legal_letters_business.id]=$moduleEntityId");
                    else
                        header("Location: ?" . http_build_query($q, '', '&'));
                } catch (Exception $e) {
                    $this->errors['__NO_FIELD__'][] = $e->getMessage();
