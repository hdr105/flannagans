...
function is_date($str){
	if (!empty($str) && trim($str) != ''){
        ...
        $value_date_array = date_parse($str);
...
function checkGroups($error) {
    $CI = & get_instance();
    $key = $CI->input->post('key');
    $data = $CI->input->post('data');
    if (empty($key)) {
        $CI->db->select('*');
        $CI->db->from('crud_groups');
        $CI->db->where('group_name', $data['crud_groups']['group_name']);

        $query = $CI->db->get();
        $rs = $query->row_array();

        if (!empty($rs)) {
            $error['crud_groups.group_name'][] = 'Role already exists. Try another? ';
        }
    }
