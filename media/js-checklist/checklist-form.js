var ScrudCForm = {
    ...
    buildFields:function(){
        $('#form-fields').append(ScrudCForm.createField('newfield'));
    },
...
//CHANGES BY KAMRAN SB START
    ...
    createField:function(field){
        ...
        $('#count').val(counts);
        field = id[0]+'-'+counts;
                ScrudCForm.addFieldToForm(field,'append',sectionId,sectionCounter);
                //////
            });
            return f;
    },
    buildOptions:function(field,pType,sectionId,sectionKey){
        ...
        var validation = $('<input class="form-control" type="checkbox" />');
        validation.append($('<label class="checkbox"> Required</label>'));
        validation.click(function(){
            if ($(this).is(':checked')) {
                sectionField.validation = 'notEmpty';
                $('#preview_field_'+field).find('.controls').children('select').attr('validation','notEmpty');
                $('#preview_error_field_'+field).show();
            }else{
                sectionField.validation = '';
                //ScrudCForm.elements[field].validation = null;
                $('#preview_field_'+field).find('.controls').children('select').attr('validation',null);
                $('#preview_error_field_'+field).hide();
            }
        });
        
        if (sectionField.validation != undefined && sectionField.validation == 'notEmpty'){
            validation.attr('checked','checked');
        }
		
        $('#preview_field_'+field).find('#form-options').append('<label class="control-label">Required</label>').append(validation);
    ...
    addBlockToForm:function(){
        ...
        if(error == false){
            ...
            ScrudCForm.addSectionToForm(id,title,'outer','default',0,'prepend'); //given by kamran
    ...
    addFieldToForm:function(field,pType,sectionId,sectionKey){
        ...
        console.log('section key data' + JSON.stringify(sectionKey));
...
////////////////
       ...
       //console.log('After sectionField: ' + JSON.stringify(ScrudCForm.elements[sectionKey].section_fields));
        ...
        var filedLabel = '';
        console.log(sectionField.label);
    ...
    */
    addSectionToForm: function(id,title,view,size,lastcount,ptype){ //chnage by kamran
        $('#check_list').remove();
        var block = $('<div class="portlet green-sharp box portlet-sortable" id="'+id+'"></div>').append($('<div class="portlet-title"><div class="caption section_head"><span>'+title+'</span><input type="hidden" name="sectionTitle" value="'+title+'" /><input type="hidden" name="viewType" value="'+view+'" /><input type="hidden" name="sizeType" value="'+size+'" /><input type="hidden" name="count" id="count" value="'+lastcount+'"></div><div class="tools"><a href="javascript:;" onclick="ScrudCForm.updateSectionTitle(\''+id+'\');" class="config" data-original-title="" title=""></a><a href="javascript:;" class="remove" onclick="ScrudCForm.removeSection(\''+id+'\');" title=""></a></div></div>')).append($('<div class="portlet-body elements_preview dragableArea sortable section_body"></div>')); //given by kamran
        if (ptype == 'append') {
 ...
 //CHANGES BY KAMRAN SB END
     buildPreviews:function(){
        ...
        for (var i = 0; i <= ScrudCForm.config.sections; i++) {
            ...
            ScrudCForm.addSectionToForm(sections[i].section_name,sections[i].section_title,sections[i].section_view,sections[i].section_size,sections[i].lastcount,'prepend'); //given by kamran
            //console.log('Lenthg of IDS2: ' + ScrudCForm.config.ids2[i].length);
            for(var a = 0; a < ScrudCForm.config.ids2[i].length; a++){
                //console.log('config ids2: ' + JSON.stringify(ScrudCForm.config.ids2[i][a]));
                
                //console.log('Field Level JSON from section:' + JSON.stringify(fields[i].section_fields));
    ...
    saveElements:function(chklst_id){
        ...
        ScrudCForm.elements = tempElements;
        ///////////////
        ...
        $('#dragable  .portlet').each(function(){    
            ...
            var lastcount = $(this).find('.section_head input[name="count"]').val(); //given by kamran
