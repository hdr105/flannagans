var ScrudCLForm = {
    ...
    init:function(table){
        if (table != undefined){
            ScrudCLForm.table = table;	    	
        }
        $('#fieldTab a[href="#options"]').hide();
    },
    
    buildFields:function(){
        $('#form-fields').append(ScrudCLForm.createField('newfield'));
    },
    ...
    addJoin:function (object){
        ...
        for(var i in ScrudCLForm.tables){
            if (ScrudCLForm.tables[i] == ScrudCLForm.table) continue; 
            joinTable.append('<option value="'+ScrudCLForm.tables[i]+'">'+ScrudCLForm.tables[i]+'</option>');
        }
        ...
        for(var i in ScrudCLForm.fields){
            currentField.append('<option value="'+ScrudCLForm.table +'.'+ScrudCLForm.fields[i]+'">'+ScrudCLForm.table +'.'+ScrudCLForm.fields[i]+'</option>');
        }
        ...
        deleteButton.click(function(){
            ...
            for(var i in ScrudCLForm.fields){
                tmpFields[tmpFields.length] = ScrudCLForm.fields[i];
            }
            ...
            for(var i in ScrudCLForm.config.column.list){
                if ($.inArray(ScrudCLForm.config.column.list[i], tmpFields) == -1){
                    removeFields[removeFields.length] = ScrudCLForm.config.column.list[i];
                }
            }
            for(var i in removeFields){
                ScrudCLForm.config.column.list.splice($.inArray(removeFields[i], ScrudCLForm.config.column.list),1);
            }
            ScrudCLForm.buildColumn();
                
        });
    	
        joinTable.change(function(){
            var targetTable = $(this).val();
            $.get(ScrudCLForm.urlGetFields+targetTable,{},function(json){
                if (json != null){
    ...
    buildFilter:function(){
        if (ScrudCLForm.config.filter == undefined){
            ScrudCLForm.config.filter = {};
            ScrudCLForm.config.filter.list = ScrudCLForm.fields;
            ScrudCLForm.config.filter.actived = [];
        }
        //if (ScrudCLForm.config.filter.list == undefined){
            ScrudCLForm.config.filter.list = ScrudCLForm.fields;
        //}
        
        /*for(var i in ScrudCLForm.fields){
            if ($.inArray(ScrudCLForm.fields[i],ScrudCLForm.config.filter.list) == -1){
                ScrudCLForm.config.filter.list[ScrudCLForm.config.filter.list.length] = ScrudCLForm.fields[i];
            }
        }*/
        
        for(var i in ScrudCLForm.config.filter.list){
            var f = $('<input type="checkbox">');
            f.val(ScrudCLForm.config.filter.list[i]);
            if (jQuery.inArray(ScrudCLForm.config.filter.list[i], ScrudCLForm.config.filter.actived) >= 0){
                f.attr({
                    checked:'checked'
                });
            }
            ...
            .attr({
                name:ScrudCLForm.config.filter.list[i]
            })
            .append($('<i class="icon-pencil"></i>'));
    		
            ebtn.click(function(){
                var field = $(this).attr('name');
                if (ScrudCLForm.config.filter.atrr == undefined){
                    ScrudCLForm.config.filter.atrr = {}
                }
                if (ScrudCLForm.config.filter.atrr[field] == undefined){
                    ScrudCLForm.config.filter.atrr[field] = {}
                }
                ...
                if (ScrudCLForm.config.filter.atrr[field].alias != undefined){
                    alias.val(ScrudCLForm.config.filter.atrr[field].alias);
                }else{
                    if (ScrudCLForm.elements[field] != undefined){
                        alias.val(ScrudCLForm.elements[field].label);
                        ScrudCLForm.config.filter.atrr[field].alias = ScrudCLForm.elements[field].label;
                    }else{
                        alias.val(field);
                        ScrudCLForm.config.filter.atrr[field].alias = field;
                    }
                }
                alias.keyup(function(){
                    ScrudCLForm.config.filter.atrr[field].alias = $(this).val();
                });
            ...
            $('#filter_elements').append(
                ...
                .append(
                    ...
                    .append(
                        ...
                        .append(f)
                        .append(ScrudCLForm.config.filter.list[i])
                        )
                    .append(ebtn)
                    )
                );
...
//CHANGES BY KAMRAN SB START
    buildQuickCreate:function(){
        var fields = ScrudCLForm.extractFields();
        //console.log('Extracted Fields: ' + JSON.stringify(ScrudCLForm.extractFields()));
        if (ScrudCLForm.config.quickcreate == undefined){
            ScrudCLForm.config.quickcreate = {};
            ScrudCLForm.config.quickcreate.list = ScrudCLForm.fields;//fields;
            ScrudCLForm.config.quickcreate.actived = [];
        }
        //if (ScrudCLForm.config.quickcreate.list == undefined){
            ScrudCLForm.config.quickcreate.list = ScrudCLForm.fields;//fields;
            //console.log(ScrudCLForm.config.quickcreate.list);
        //}
        ...
        for(var i in ScrudCLForm.config.quickcreate.list){
            var f = $('<input type="checkbox">');
            f.val(ScrudCLForm.config.quickcreate.list[i]);
            if (jQuery.inArray(ScrudCLForm.config.quickcreate.list[i], ScrudCLForm.config.quickcreate.actived) >= 0){
                f.attr({
                    checked:'checked'
                });
            }
            ...
            .attr({
                name:ScrudCLForm.config.quickcreate.list[i]
            })
            .append($('<i class="icon-pencil"></i>'));
            
            ebtn.click(function(){
                var field = $(this).attr('name');
                if (ScrudCLForm.config.quickcreate.atrr == undefined){
                    ScrudCLForm.config.quickcreate.atrr = {}
                }
                if (ScrudCLForm.config.quickcreate.atrr[field] == undefined){
                    ScrudCLForm.config.quickcreate.atrr[field] = {}
                }
                ...
                if (ScrudCLForm.config.quickcreate.atrr[field].alias != undefined){
                    alias.val(ScrudCLForm.config.quickcreate.atrr[field].alias);
                }else{
                    if (fields[field] != undefined){
                        alias.val(fields[field].label);
                        ScrudCLForm.config.quickcreate.atrr[field].alias = fields[field].label;
                    }else{
                        alias.val(field);
                        ScrudCLForm.config.quickcreate.atrr[field].alias = field;
                    }
                }
                alias.keyup(function(){
                    ScrudCLForm.config.quickcreate.atrr[field].alias = $(this).val();
                });
            ...
            $('#quickcreate_elements').append(
                ...
                .append(
                    ...
                    .append(
                        $('<label class="checkbox"></label>')
                        .append(f)
                        .append(ScrudCLForm.config.quickcreate.list[i])
                        )
                    .append(ebtn)
                    )
                );
        ...
        //console.log('quick created list: ' + JSON.stringify(ScrudCLForm.config.quickcreate.list));
    },
    buildMassEdit:function(){
        var fields = ScrudCLForm.extractFields();
        //console.log('Extracted Fields: ' + JSON.stringify(ScrudCLForm.extractFields()));
        if (ScrudCLForm.config.massedit == undefined){
            ScrudCLForm.config.massedit = {};
            ScrudCLForm.config.massedit.list = ScrudCLForm.fields;//fields;
            ScrudCLForm.config.massedit.actived = [];
        }
        //if (ScrudCLForm.config.quickcreate.list == undefined){
            ScrudCLForm.config.massedit.list = ScrudCLForm.fields;//fields;
            //console.log(ScrudCLForm.config.massedit.list);
        //}
        ...
        for(var i in ScrudCLForm.config.massedit.list){
            var f = $('<input type="checkbox">');
            f.val(ScrudCLForm.config.massedit.list[i]);
            if (jQuery.inArray(ScrudCLForm.config.massedit.list[i], ScrudCLForm.config.massedit.actived) >= 0){
                f.attr({
                    checked:'checked'
                });
            }
            var ebtn = $('<a class="btn btn-sm blue" style="float: right; cursor:pointer;"></a>')
            .attr({
                name:ScrudCLForm.config.massedit.list[i]
            })
            .append($('<i class="icon-pencil"></i>'));
            
            ebtn.click(function(){
                var field = $(this).attr('name');
                if (ScrudCLForm.config.massedit.atrr == undefined){
                    ScrudCLForm.config.massedit.atrr = {}
                }
                if (ScrudCLForm.config.massedit.atrr[field] == undefined){
                    ScrudCLForm.config.massedit.atrr[field] = {}
                }
                ...
                if (ScrudCLForm.config.massedit.atrr[field].alias != undefined){
                    alias.val(ScrudCLForm.config.massedit.atrr[field].alias);
                }else{
                    if (fields[field] != undefined){
                        alias.val(fields[field].label);
                        ScrudCLForm.config.massedit.atrr[field].alias = fields[field].label;
                    }else{
                        alias.val(field);
                        ScrudCLForm.config.massedit.atrr[field].alias = field;
                    }
                }
                alias.keyup(function(){
                    ScrudCLForm.config.massedit.atrr[field].alias = $(this).val();
                });
            ...
            $('#massedit_elements').append(
                ...
                .append(
                    ...
                    .append(
                        $('<label class="checkbox"></label>')
                        .append(f)
                        .append(ScrudCLForm.config.massedit.list[i])
                        )
                    .append(ebtn)
                    )
                );
        ...
        //console.log('quick created list: ' + JSON.stringify(ScrudCLForm.config.massedit.list));
    },
    buildSummaryView:function(){
        var fields = ScrudCLForm.extractFields();
        //console.log('Extracted Fields: ' + JSON.stringify(ScrudCLForm.extractFields()));
        if (ScrudCLForm.config.summary == undefined){
            ScrudCLForm.config.summary = {};
            ScrudCLForm.config.summary.list = ScrudCLForm.fields;//fields;
            ScrudCLForm.config.summary.actived = [];
        }
        //if (ScrudCLForm.config.quickcreate.list == undefined){
            ScrudCLForm.config.summary.list = ScrudCLForm.fields;//fields;
            //console.log(ScrudCLForm.config.massedit.list);
        ...
        for(var i in ScrudCLForm.config.summary.list){
            var f = $('<input type="checkbox">');
            f.val(ScrudCLForm.config.summary.list[i]);
            if (jQuery.inArray(ScrudCLForm.config.summary.list[i], ScrudCLForm.config.summary.actived) >= 0){
                f.attr({
                    checked:'checked'
                });
            }
            var ebtn = $('<a class="btn btn-sm blue" style="float: right; cursor:pointer;"></a>')
            .attr({
                name:ScrudCLForm.config.summary.list[i]
            })
            .append($('<i class="icon-pencil"></i>'));
            
            ebtn.click(function(){
                var field = $(this).attr('name');
                if (ScrudCLForm.config.summary.atrr == undefined){
                    ScrudCLForm.config.summary.atrr = {}
                }
                if (ScrudCLForm.config.summary.atrr[field] == undefined){
                    ScrudCLForm.config.summary.atrr[field] = {}
                }
                ...
                if (ScrudCLForm.config.summary.atrr[field].alias != undefined){
                    alias.val(ScrudCLForm.config.summary.atrr[field].alias);
                }else{
                    if (fields[field] != undefined){
                        alias.val(fields[field].label);
                        ScrudCLForm.config.summary.atrr[field].alias = fields[field].label;
                    }else{
                        alias.val(field);
                        ScrudCLForm.config.summary.atrr[field].alias = field;
                    }
                }
                alias.keyup(function(){
                    ScrudCLForm.config.summary.atrr[field].alias = $(this).val();
                });
            ...
            $('#summary_elements').append(
                ...
                .append(
                    ...
                    .append(
                        $('<label class="checkbox"></label>')
                        .append(f)
                        .append(ScrudCLForm.config.summary.list[i])
                        )
                    .append(ebtn)
                    )
                );
    ...
    //CHANGES BY KAMRAN SB END
      ...
      buildColumn:function(){
        ...
        if (ScrudCLForm.config.column == undefined){
            ScrudCLForm.config.column = {};
            ScrudCLForm.config.column.list = ScrudCLForm.fields;
            ScrudCLForm.config.column.actived = [];
        }
        if (ScrudCLForm.config.column.list == undefined){
            ScrudCLForm.config.column.list = ScrudCLForm.fields;
        }
        
        for(var i in ScrudCLForm.fields){
            if ($.inArray(ScrudCLForm.fields[i],ScrudCLForm.config.column.list) == -1){
                ScrudCLForm.config.column.list[ScrudCLForm.config.column.list.length] = ScrudCLForm.fields[i];
            }
        }
        
        
        $('#dataListJoin > div > #targetField > option').each(function(){
            if ($.inArray($(this).val(), ScrudCLForm.config.column.list) == -1){
                ScrudCLForm.config.column.list[ScrudCLForm.config.column.list.length] = $(this).val();
            }
        });
        
        for(var i in ScrudCLForm.config.column.list){
            var f = $('<input type="checkbox">');
            f.val(ScrudCLForm.config.column.list[i]);
            if (jQuery.inArray(ScrudCLForm.config.column.list[i], ScrudCLForm.config.column.actived) >= 0){
                f.attr({
                    checked:'checked'
                });
            }
            ...
            var ebtn = $('<a class="btn btn-mini" style="float: right; cursor:pointer;"></a>')
            .attr({
                name:ScrudCLForm.config.column.list[i]
            })
            .append($('<i class="icon-pencil"></i>'));
    		 
            ebtn.click(function(){
                var field = $(this).attr('name');
                if (ScrudCLForm.config.column.atrr == undefined){
                    ScrudCLForm.config.column.atrr = {}
                }
                if (ScrudCLForm.config.column.atrr[field] == undefined){
                    ScrudCLForm.config.column.atrr[field] = {}
                }
                ...
                if (ScrudCLForm.config.column.atrr[field].alias != undefined){
                    alias.val(ScrudCLForm.config.column.atrr[field].alias);
                }else{
                    if (ScrudCLForm.elements[field] != undefined){
                        alias.val(ScrudCLForm.elements[field].label);
                        ScrudCLForm.config.column.atrr[field].alias = ScrudCLForm.elements[field].label;
                    }else{
                        alias.val(field);
                        ScrudCLForm.config.column.atrr[field].alias = field;
                    }
                }
                alias.keyup(function(){
                    ScrudCLForm.config.column.atrr[field].alias = $(this).val();
                });
                ...
                if (ScrudCLForm.config.column.atrr[field].format != undefined){
                    format.val(ScrudCLForm.config.column.atrr[field].format);
                }
                format.keyup(function(){
                    ScrudCLForm.config.column.atrr[field].format = $(this).val();
                });
                ...
                if (ScrudCLForm.config.column.atrr[field].width != undefined){
                    width.val(ScrudCLForm.config.column.atrr[field].width);
                }
                width.keyup(function(){
                    ScrudCLForm.config.column.atrr[field].width = $(this).val();
                });
                ...
                la.change(function(){
                    ScrudCLForm.config.column.atrr[field].align = $(this).val();
                });
                ...
                if (ScrudCLForm.config.column.atrr[field].align != undefined){
                    la.val(ScrudCLForm.config.column.atrr[field].align);
                }
            ...
            $('#column_elements').append(
                ...
                .append(
                    ...
                    .append(
                        $('<label class="checkbox"></label>')
                        .append(f)
                        .append(ScrudCLForm.config.column.list[i])
                        )
                    .append(ebtn)
                    )
                );
    ...
    buildConfigTable:function(){
        if (ScrudCLForm.config.table == undefined){
            ScrudCLForm.config.table = {};
        }
        if (ScrudCLForm.config.table.crudTitle != undefined){
            $('#crudTitle').val(ScrudCLForm.config.table.crudTitle);
        }else{
            $('#crudTitle').val(ScrudCLForm.table+' manager');
            ScrudCLForm.config.table.crudTitle = $('#crudTitle').val();
        }
        if (ScrudCLForm.config.table.crudRowsPerPage != undefined){
            $('#crudRowsPerPage').val(ScrudCLForm.config.table.crudRowsPerPage);
        }else{
            $('#crudRowsPerPage').val(20);
            ScrudCLForm.config.table.crudRowsPerPage = $('#crudRowsPerPage').val();
        }
        if (ScrudCLForm.config.table.crudOrderField != undefined){
            $('#crudOrderField').val(ScrudCLForm.config.table.crudOrderField);
        }
        if (ScrudCLForm.config.table.crudOrderType != undefined){
            $('#crudOrderType').val(ScrudCLForm.config.table.crudOrderType);
        }
        if (ScrudCLForm.config.table.noColumn != undefined && 
            ScrudCLForm.config.table.noColumn == 1){
            //$('#crudNoColumn').val(ScrudCLForm.config.table.noColumn);
            ScrudCLForm.config.table.noColumn = 1;
            $('#crudNoColumn').attr({
                checked:'checked'
            });
        ...
        $('#crudTitle').keyup(function(){
            ScrudCLForm.config.table.crudTitle = $(this).val();
        });
        
        $('#crudRowsPerPage').keyup(function(){
            ScrudCLForm.config.table.crudRowsPerPage = $(this).val();
        });
        $('#crudOrderField').change(function(){
            ScrudCLForm.config.table.crudOrderField = $(this).val();
        });
        $('#crudOrderType').change(function(){
            ScrudCLForm.config.table.crudOrderType = $(this).val();
        });
        $('#crudNoColumn').click(function(){
            if ($('#crudNoColumn').attr('checked') == 'checked'){
                ScrudCLForm.config.table.noColumn = 1;
            }else{
                ScrudCLForm.config.table.noColumn = 0;
            }
        })
    },
        ...
        if(field=="newfield"){
            ...
            f.click(function(){
                /*if (!$(this).hasClass('disabled')){
                    ScrudCLForm.addFieldToForm(field,'prepend');
                }*/
                ...
                //ScrudCLForm.addFieldToForm(field,'prepend',sectionId,0); //for update old sections
                ScrudCLForm.addFieldToFormEmpty(field,'prepend',sectionId,sectionCounter);
                //////
            });
            return f;
        }else{
            ...
            f.click(function(){
                ...
                sectionCounter = sectionCounter -1;
                //ScrudCLForm.addFieldToForm(field,'prepend',sectionId,0); //for update old sections
                ScrudCLForm.addFieldToForm(field,'prepend',sectionId,sectionCounter);
                //////
            });
            return f;
        }
    ...
    buildOptions:function(field,pType,sectionId,sectionKey){
        ...
        var sectionField = ScrudCLForm.elements[sectionKey].section_fields[field];
       ...
       ////////////////////////////////////////////////////////////////////
        ...
        for(var k in ScrudCLForm.fields){
            if(sectionField.fieldattached == ScrudCLForm.fields[k]){
                fieldattached.append($('<option selected="selected" value="'+ScrudCLForm.fields[k]+'">'+ScrudCLForm.fields[k]+'</option>'));
            } else {
                fieldattached.append($('<option value="'+ScrudCLForm.fields[k]+'">'+ScrudCLForm.fields[k]+'</option>'));
            }  
        }
        ...
        type.change(function(){
            ...
            ScrudCLForm.changeFieldToForm(field,pType,sectionId,sectionKey);
            $('#type-options').html('');
            switch (sectionField.type){
                ...
                case 'currency':
                    ...
                    break;
                case 'comments':
                    ScrudCLForm.removeFieldsInSection(field,sectionId);
                    textOptions(field,pType,sectionId,sectionKey,typeOptions);
                    break;
                //Currency
    ...
    addBlockToForm:function(){
        ...
        if(error == false){
            ...
            //ScrudCLForm.addSectionToForm(id,title,'prepend');
            ScrudCLForm.addSectionToForm(id,title,'outer','default','prepend'); //given by kamran
    ...
    //empty function
       ...
       // console.log('Before undefined: ' + JSON.stringify(ScrudCLForm.elements));
        if (ScrudCLForm.elements[sectionKey] == undefined){
...
////////////////
            ScrudCLForm.elements[sectionKey] = {
            ...
            ScrudCLForm.elements[sectionKey].section_fields[field] = {
        ...
        if (ScrudCLForm.elements[sectionKey].section_fields[field] == undefined) {
            ScrudCLForm.elements[sectionKey].section_fields[field] = {
         ...
         console.log('After assigning values: ' + JSON.stringify(ScrudCLForm.elements));
       var sectionField = ScrudCLForm.elements[sectionKey].section_fields[field];
       //console.log('After sectionField: ' + JSON.stringify(ScrudCLForm.elements[sectionKey].section_fields));
        ...
        iedit.clickover({
            ...
            onShown:function(){
                ScrudCLForm.buildOptions(field,pType,sectionId,sectionKey);
            }
        }); 
        ...
        ir.click(function(){
            ...
            //$('#form-preview').height($('#form-preview').height());
            if (ScrudCLForm.current_field == field){
        ...
        switch (sectionField.type){
            ...
            case 'select':
                ...
                if (sectionField.list_choose == 'default'){
                    ...
                    $.post(ScrudCLForm.urlGetOptions,{
        ...
        iedit.click(function(){
            ...
            ScrudCLForm.current_field = field;
        ...
        li.click(function(){
            if (dragFlag == false){
                ...
                //$('#form-options').html('');
                //ScrudCLForm.buildOptions(field);
...
////////////////
    ...
    /*addSectionToForm: function(id,title,ptype){
        var block = $('<div class="portlet green-sharp box" id="'+id+'"></div>').append($('<div class="portlet-title"><div class="caption section_head"><span>'+title+'</span><input type="hidden" name="sectionTitle" value="'+title+'" /><input type="hidden" name="viewType" value="outer" /></div><div class="tools"><a href="javascript:;" onclick="ScrudCLForm.updateSectionTitle(\''+id+'\');" class="config" data-original-title="" title=""></a><a href="javascript:;" class="remove" onclick="ScrudCLForm.removeSection(\''+id+'\');" title=""></a></div></div>')).append($('<div class="portlet-body elements_preview dragableArea sortable section_body"></div>'));
    */
    addSectionToForm: function(id,title,view,size,ptype){ //chnage by kamran
        
        var block = $('<div class="portlet green-sharp box portlet-sortable" id="'+id+'"></div>').append($('<div class="portlet-title"><div class="caption section_head"><span>'+title+'</span><input type="hidden" name="sectionTitle" value="'+title+'" /><input type="hidden" name="viewType" value="'+view+'" /><input type="hidden" name="sizeType" value="'+size+'" /></div><div class="tools"><a href="javascript:;" onclick="ScrudCLForm.updateSectionTitle(\''+id+'\');" class="config" data-original-title="" title=""></a><a href="javascript:;" class="remove" onclick="ScrudCLForm.removeSection(\''+id+'\');" title=""></a></div></div>')).append($('<div class="portlet-body elements_preview dragableArea sortable section_body"></div>')); //given by kamran
        if (ptype == 'append') {
...
//CHANGES BY KAMRAN SB STart
 extractFields:function(){
        var fieldsArr = [];
        var fields = ScrudCLForm.elements;
        var sections = ScrudCLForm.elements;
       
        for (var i = 0; i <= ScrudCLForm.config.sections; i++) {
            
            
            for(var a = 0; a < ScrudCLForm.config.ids2[i].length; a++){
                
                if (fields[i].section_fields[ScrudCLForm.config.ids2[i][a]] != undefined){
                   
                    fieldsArr.push(fields[i].section_fields[ScrudCLForm.config.ids2[i][a]].field);
 ...
 //CHANGES BY KAMRAN SB END
     buildPreviews:function(){
        var fields = ScrudCLForm.elements;
        var sections = ScrudCLForm.elements;
        var Cfunctions = ScrudCLForm.config.functions;
		
		//CHANGE HERE TO VIEW BUILD START
        if (ScrudCLForm.config.functions != undefined){
        ...
        /* if (ScrudCLForm.config.ids == undefined){
            ScrudCLForm.config.ids = [];
            for(var i in fields){
                ScrudCLForm.config.ids[ScrudCLForm.config.ids.length] = i;
            }
        }*/
        for (var i = 0; i <= ScrudCLForm.config.sections; i++) {
            //ScrudCLForm.addSectionToForm(sections[i].section_name,sections[i].section_title,'prepend');
            ScrudCLForm.addSectionToForm(sections[i].section_name,sections[i].section_title,sections[i].section_view,sections[i].section_size,'prepend'); //given by kamran
            //console.log('Lenthg of IDS2: ' + ScrudCLForm.config.ids2[i].length);
            for(var a = 0; a < ScrudCLForm.config.ids2[i].length; a++){
                //console.log('conifig ids2: ' + JSON.stringify(ScrudCLForm.config.ids2[i][a]));
                
                //console.log('Field Level JSAON from section:' + JSON.stringify(fields[i].section_fields));

                if (fields[i].section_fields[ScrudCLForm.config.ids2[i][a]] != undefined){
                   
                    ScrudCLForm.addFieldToForm(fields[i].section_fields[ScrudCLForm.config.ids2[i][a]].field,fields[i].section_fields[ScrudCLForm.config.ids2[i][a]].label,sections[i].section_name,i);
                    //console.log(fields[i].section_fields[ScrudCLForm.config.ids2[i][a]].label+' section: ' + i);
                }
            }
        }
    ...
    saveElements:function(table,com_id){
        ...
        ScrudCLForm.config.filter.list = [];
        $('#filter_elements').find('input').each(function(){
            ScrudCLForm.config.filter.list[ScrudCLForm.config.filter.list.length] = $(this).val();
        });
        
        ScrudCLForm.config.filter.actived = [];
        $('#filter_elements').find('input:checked').each(function(){
            ScrudCLForm.config.filter.actived[ScrudCLForm.config.filter.actived.length] = $(this).val();
        });
        
        //UPDATE BY KAMRAN SB START
        ScrudCLForm.config.quickcreate.list = [];
        $('#quickcreate_elements').find('input').each(function(){
            ScrudCLForm.config.quickcreate.list[ScrudCLForm.config.quickcreate.list.length] = $(this).val();
        });
        
        ScrudCLForm.config.quickcreate.actived = [];
        $('#quickcreate_elements').find('input:checked').each(function(){
            ScrudCLForm.config.quickcreate.actived[ScrudCLForm.config.quickcreate.actived.length] = $(this).val();
        });

        ScrudCLForm.config.massedit.list = [];
        $('#massedit_elements').find('input').each(function(){
            ScrudCLForm.config.massedit.list[ScrudCLForm.config.massedit.list.length] = $(this).val();
        });
        ScrudCLForm.config.massedit.actived = [];
        $('#massedit_elements').find('input:checked').each(function(){
            ScrudCLForm.config.massedit.actived[ScrudCLForm.config.massedit.actived.length] = $(this).val();
        });

        ScrudCLForm.config.summary.list = [];
        $('#summary_elements').find('input').each(function(){
            ScrudCLForm.config.summary.list[ScrudCLForm.config.summary.list.length] = $(this).val();
        });
        ScrudCLForm.config.summary.actived = [];
        $('#summary_elements').find('input:checked').each(function(){
            ScrudCLForm.config.summary.actived[ScrudCLForm.config.summary.actived.length] = $(this).val();
        });
        //UPDATE BY KAMRAN SB END



        ScrudCLForm.config.column.list = [];
        ...
        $('#column_elements').find('input:checked').each(function(){
            ScrudCLForm.config.column.actived[ScrudCLForm.config.column.actived.length] = $(this).val();
        });
		
		ScrudCLForm.config.join = [];
        ...
        $('#dataListJoin > div').each(function(){
            ...
            ScrudCLForm.config.join[ScrudCLForm.config.join.length] = object;
        });
        ...
        $('.elements_preview > div').each(function(){
            ...
            ids[ids.length] = id;
            if (ScrudCLForm.elements[id] != undefined){
                elements[id] = ScrudCLForm.elements[id];
            }
        });
        var config = ScrudCLForm.config;
        config.ids = ids;
        ...
        var kam = 0;
        var tempElements = {};
        console.log('From elements: '+JSON.stringify(ScrudCLForm.elements));
        $('#dragable > div').each(function(){
            ...
            //console.log('section title: '+section_name+', From elements: '+JSON.stringify(ScrudCLForm.elements[sectionCounter].section_name));
            for (var i = 0; i < sectionCounter; i++) {
                if (ScrudCLForm.elements[i] != undefined) {
                    if (section_name == ScrudCLForm.elements[i].section_name) {
                        tempElements[kam] = ScrudCLForm.elements[i];
                        //console.log('found at: ' +kam +' of: ' +i);
                        //delete ScrudCLForm.elements[i];
                    }
                }
        ...
        ScrudCLForm.elements = tempElements;
        ///////////////

        var sectionCounter = $('#dragable').length;
        console.log('section count: ' + sectionCounter);
        var simpleCounter = 0 ;
        console.log('all elements field: ' + JSON.stringify(ScrudCLForm.elements));
        ...
        $('#dragable  .portlet').each(function(){    
            ...
            $(this).find('.elements_preview > .row').each(function(){
                ...
                ids[ids.length] = id;
                if (ScrudCLForm.elements[simpleCounter] != undefined){
                    var sectionField = ScrudCLForm.elements[simpleCounter].section_fields[id];
		...
		config.ids2 = ids2;
        config.sections = simpleCounter-1;
        ...
        ScrudCLForm.successfully_message +
        '</div>';
        $.post(ScrudCLForm.urlSaveConfig, {
            scrud:sections,
            table:table,
            com_id:com_id,
            config:config
...
$(document).ready(function(){
    ...
    $('#btn_field_to_form').clickover({
        ...
        onShown:function(){
            ScrudCLForm.buildFields();
        }
    }); 
	        
    ScrudCLForm.init();
    ScrudCLForm.buildPreviews();
    ScrudCLForm.buildConfigTable();
    ScrudCLForm.buildFilter();
    ScrudCLForm.buildColumn();

    //UPDATE BY KAMRAN SB START
    ScrudCLForm.buildQuickCreate();
    ScrudCLForm.buildMassEdit();
    ScrudCLForm.buildSummaryView();
    //UPDATE BY KAMRAN SB END
    
    /*$('#btnSaveDataList').click(function(){
        ScrudCLForm.saveElements(ScrudCLForm.table);
    });*/
    ...
    if (ScrudCLForm.config.join != undefined){
        //        console.log(ScrudCLForm.config.join);
        for(var i in ScrudCLForm.config.join){
            ScrudCLForm.addJoin(ScrudCLForm.config.join[i]);
        }
    }
});
